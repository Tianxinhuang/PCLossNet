...
def decoder_with_fc_only(latent_signal, layer_sizes=[],local=False, b_norm=True,b_norm_decay=0.9,b_norm_decay_finish=0.9, non_linearity=tf.nn.relu,
    ...
    return layer
def decoder_with_folding_only(latent_signal, layer_sizes=[],local=False, b_norm=True,b_norm_decay=0.9,b_norm_decay_finish=0.9, non_linearity=tf.nn.relu,
                         regularizer=None, weight_decay=0.001, reuse=False, scope=None, dropout_prob=None,
                         b_norm_finish=False, verbose=False):
    '''A decoding network which maps points from the latent space back onto the data space.
    '''
    ...
    if local:
        ...
        mlp22=[256,256]
    for i, outchannel in enumerate(mlp22):
